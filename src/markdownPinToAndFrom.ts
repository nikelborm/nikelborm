...
function parsePathToImageInsideRepo(pathToImageInsideRepo: string) {
  ...
  if (!result.success) throw new Error(
    'Image filename doesn\'t follow naming convention.',
    { cause: result.error }
  );
...
const imageFileNameElements = z.object({
...
const MarkdownRepoPinZodSchema = z.object({
  ...
  repoPinImageURL: z.string().min(50).url().transform(val => new URL(val)),
...
function parseMarkdownPinRegexMatchGroup(
  ...
  if (repoPinImageURL.host === 'github-readme-stats.vercel.app' as const) {
    ...
    if (!repoNameFromPinURL) throw new Error(outdent`
    ...
    if (!sameOwner) throw new Error(outdent`
    ...
    if (owner !== sameOwner) throw new Error(outdent`
  ...
  if (repoURL.host !== 'github.com') throw new Error(
    `Repo URL isn't even on github.com`
  );
  ...
  if (!repoNameFromRepoURL) throw new Error(
    `Repo URL doesn't have repoName in it`
  );

  if (repoNameFromRepoURL !== repoNameFromPinURL) throw new Error(
    `Repo name from repo URL is not equal to repo name from pin image URL`
  );

  if (usernameFromRepoURL !== usernameFromPinURL) throw new Error(
